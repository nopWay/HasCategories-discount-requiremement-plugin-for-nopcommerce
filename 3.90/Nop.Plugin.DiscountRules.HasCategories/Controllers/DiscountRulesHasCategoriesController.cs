...
namespace Nop.Plugin.DiscountRules.HasCategories.Controllers
{
    ...
    public class DiscountRulesHasCategoriesController : BasePluginController
    {
        ...
        [HttpPost]
        [AdminAntiForgery]
        public ActionResult Configure(int discountId, int? discountRequirementId, int productQuantityMin, int productQuantityMax, string categoryIds, string excludedProductIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageDiscounts))
